...
export function getLinesBySection(sectionId) {
  return request.get(`/lines/section/${sectionId}`)
}
...
// 导出结果和字幕
export function exportLines(chapter_id, single = false, export_dir = null) {
  return request.post(`/lines/export-audio/${chapter_id}`, {
    single,
    export_dir
  })
}
...
// 批量替换角色
export function batchReplaceRole(data) {
  return request.put('/lines/batch/replace-role', data)
}