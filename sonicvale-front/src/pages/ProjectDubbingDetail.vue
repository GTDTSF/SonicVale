<template>
    <div class="page-wrap">
        ...
        <el-container class="main">
            <!-- 左侧章节 -->
            <el-aside :width="asideCollapsed ? '0px' : '280px'" class="aside" :class="{ 'aside-collapsed': asideCollapsed }">
                    ...
                    <div class="aside-title">
                        ...
                        <div class="title-right">
                            <el-tooltip content="定位到当前章节" placement="top">
                                <el-button circle size="small" type="primary" plain @click="expandedChapterId = activeChapterId">
                                    <el-icon>
                                        <Refresh />
                                    </el-icon>
                                </el-button>
                ...
                <div class="chapter-list-container">
                    <el-collapse v-model="expandedChapterId" accordion @change="onChapterCollapseChange">
                        <el-collapse-item v-for="ch in filteredChapters" :key="ch.id" :name="ch.id">
                            <template #title>
                                <div class="chapter-collapse-title" :class="{ 'is-active': activeChapterId === ch.id && !activeSectionId }">
                                    <el-icon><Folder /></el-icon>
                                    <span class="ellipsis">{{ ch.title }}</span>
                                    <div class="chapter-arrow-btns" @click.stop>
                                        <el-icon :size="14" @click="handleChapterUp(ch)" class="arr-icon" title="上移"><ArrowUp /></el-icon>
                                        <el-icon :size="14" @click="handleChapterDown(ch)" class="arr-icon" title="下移"><ArrowDown /></el-icon>
                                    </div>
                                </div>
                            </template>
                            <div class="chapter-collapse-body">
                                ...
                                <div class="section-item"
                                     :class="{ 'is-active': activeChapterId === ch.id && !activeSectionId }"
                                     @click.stop="handleSelectChapter(ch)">
                                    <el-icon><Document /></el-icon>
                                    <span>全部台词</span>
                                </div>

                                <!-- 分段列表 -->
                                <div v-for="sec in sectionMap[ch.id] || []" :key="sec.id"
                                     class="section-item"
                                     :class="{ 'is-active': activeSectionId === sec.id }"
                                     @click.stop="handleSelectSection(sec, ch)">
                                    <el-icon><Notebook /></el-icon>
                                    <span class="ellipsis">{{ sec.title }}</span>
                                    <div class="section-ops">
                                        <el-button link size="small" @click.stop="openRenameSection(sec)">
                                            <el-icon><Edit /></el-icon>
                                        </el-button>
                                        <el-popconfirm title="确认删除该分段？" @confirm="handleDeleteSection(sec, ch)">
                                            <template #reference>
                                ...
                                <!-- 新建分段 -->
                                <div class="section-item add-section" @click.stop="openNewSectionDialog(ch)">
                                    <el-icon><Plus /></el-icon>
                                    <span>新建分段</span>
                                </div>

                                <div class="chapter-ops-separator"></div>

                                <!-- 章节操作 -->
                                <el-button size="small" plain @click.stop="openRenameChapter(ch)">
                                    <el-icon><Edit /></el-icon>重命名
                                </el-button>
                                <el-popconfirm title="确认删除该章节？" @confirm="deleteChapter(ch)">
                                    <template #reference>
                    ...
                    <el-empty v-if="filteredChapters.length === 0 && chapters.length === 0" description="暂无章节" />
                    <el-empty v-else-if="filteredChapters.length === 0" description="无匹配章节" />
                </div>
            ...
            <el-main class="content">
                ...
                <el-card class="chapter-card">
                    <div class="chapter-card-head">
                        <div class="left">
                            ...
                            <span class="title">
                                {{ activeSectionId && sectionMap[activeChapterId]?.find(s => s.id === activeSectionId)?.title
                        ...
                        <div class="right">
                            ...
                            <el-button @click="autoSplit" text :disabled="!currentChapterContent">
                                <el-icon>
                                    <Operation />
                                </el-icon> 自动分段
                            </el-button>
                ...
                <el-tabs v-model="activeTab" class="el-tabs-box">
                        ...
                        <div class="toolbar">
                            ...
                            <div class="toolbar-group">
                                <el-select v-model="roleFilter" clearable filterable placeholder="按角色筛选" class="filter-item w200">
                                    <el-option v-for="r in chapterRoles" :key="r.id" :label="r.name" :value="r.id" />
                                </el-select>
                    ...
                    <el-tab-pane label="角色库" name="roles">
                        ...
                        <div class="toolbar">
                            <el-radio-group v-model="roleFilterMode" size="small">
                                <el-radio-button value="all">全部角色</el-radio-button>
                                <el-radio-button value="chapter">本章角色</el-radio-button>
                            </el-radio-group>
                            <el-input v-model="roleKeyword" placeholder="搜索角色" clearable class="w260" style="margin-left:8px">
                                <template #prefix>
                                    <el-icon>
                                        <Search />
                                    </el-icon>
                            ...
                            <el-button type="warning" plain @click="openReplaceRole">
                                <el-icon class="mr-1">
                                    <Refresh />
                                </el-icon> 替换角色
                            </el-button>
        ...
        <!-- 新建/重命名分段 -->
        <el-dialog :title="sectionEditingId ? '重命名分段' : '新建分段'" v-model="sectionDialogVisible" width="460px">
            <el-form :model="sectionForm" label-width="90px">
                <el-form-item label="分段名称" prop="title"
                    :rules="[{ required: true, message: '请输入分段名称', trigger: 'blur' }]">
                    <el-input v-model="sectionForm.title" @keyup.enter="handleCreateSection" placeholder="例如：第1幕" />
                </el-form-item>
            </el-form>
            <template #footer>
                <el-button @click="sectionDialogVisible = false">取消</el-button>
                <el-button type="primary" @click="handleCreateSection">确定</el-button>
            </template>
        </el-dialog>
        ...
        <!-- 批量替换角色 -->
        <el-dialog v-model="dialogReplaceRole" title="批量替换角色" width="460px">
            <el-form :model="replaceRoleForm" ref="replaceRoleFormRef" label-width="100px">
                <el-form-item label="当前章节" v-if="!activeChapterId">
                    <el-tag type="danger">请先在左侧选择一个章节</el-tag>
                </el-form-item>
                <el-form-item label="将角色" prop="from_role_id"
                    :rules="[{ required: true, message: '请选择要替换的角色', trigger: 'change' }]">
                    <el-select v-model="replaceRoleForm.from_role_id" filterable placeholder="选择原角色" style="width:100%">
                        <el-option v-for="r in roles" :key="r.id" :label="r.name" :value="r.id" />
                    </el-select>
                </el-form-item>
                <el-form-item label="替换为" prop="to_role_id"
                    :rules="[{ required: true, message: '请选择目标角色', trigger: 'change' }]">
                    <el-select v-model="replaceRoleForm.to_role_id" filterable placeholder="选择目标角色" style="width:100%">
                        <el-option v-for="r in roles" :key="r.id" :label="r.name" :value="r.id" />
                    </el-select>
                </el-form-item>
            </el-form>
            <template #footer>
                <el-button @click="dialogReplaceRole = false">取消</el-button>
                <el-button type="primary" @click="handleReplaceRole" :disabled="!activeChapterId">替换</el-button>
            </template>
        </el-dialog>
        <!-- 项目设置弹窗（复用创建项目表单结构） -->
...
import {
    ...
    CircleCheck, CircleClose, QuestionFilled, Odometer, Microphone, ArrowUp, ArrowDown, Operation, DArrowLeft, DArrowRight, View, Folder, Notebook
            ...
            if (msg.event === 'line_update') {
                if (msg.status !== 'queued' && msg.status !== 'processing') {
                    const type = msg.status === 'failed' ? 'danger'
                        : msg.status === 'done' ? 'success' : 'info'
                    const meta = msg.meta || (msg.status === 'done' ? '生成完成' : msg.status === 'failed' ? '生成失败' : '状态更新')
...
const currentChapterContent = computed(() => {
    if (activeSectionId.value && currentSectionText.value) return currentSectionText.value
    return currentChapter.value?.text_content || ''
})
...
function loadChapterContent(indexStr) {
    ...
    expandedChapterId.value = activeChapterId.value
    loadLines()
...
const onChapterCollapseChange = (val) => {
    if (val) {
        loadSections(val)
        if (val !== activeChapterId.value) {
            handleSelectChapter(chapters.value.find(c => c.id === val))
        }
    }
}
...
async function createChapter() {
    ...
    if (res?.code === 200) {
        ...
        await loadChapters()
    } else if (res?.code === 400 && res?.message?.includes('已存在')) {
        try {
            await ElMessageBox.confirm(
                `章节「${title}」已存在（可能是之前删除未彻底），是否删除旧章节并重新创建？`,
                '章节重名',
                { confirmButtonText: '删除并重建', cancelButtonText: '取消', type: 'warning' }
            )
            const dup = chapters.value.find(c => c.title === title)
            if (dup) {
                await chapterAPI.deleteChapter(dup.id)
                await loadChapters()
            }
            const retry = await chapterAPI.createChapter(title, projectId)
            if (retry?.code === 200) {
                ElMessage.success('已创建章节')
                dialogNewChapter.value = false
                chapterForm.value = { id: null, title: '' }
                ...
                ElMessage.error(retry?.message || '创建失败')
            }
        } catch { /* 用户取消 */ }
    } else {
...
async function handleChapterUp(ch) {
    try {
        const res = await chapterAPI.reorderChapter(ch.id, 'up')
        if (res?.code === 200) {
            await loadChapters()
        } else {
            ElMessage.warning(res?.message || '无法上移')
        }
    } catch (e) {
...
async function handleChapterDown(ch) {
    try {
        const res = await chapterAPI.reorderChapter(ch.id, 'down')
        if (res?.code === 200) {
            await loadChapters()
        } else {
            ElMessage.warning(res?.message || '无法下移')
        }
    } catch (e) {
        ElMessage.error('操作失败')
    }
...
async function loadSections(chapterId) {
    const res = await sectionAPI.getSectionsByChapter(chapterId)
    sectionMap[chapterId] = res?.code === 200 ? (res.data || []) : []
}

const handleSelectSection = async (sec, ch) => {
    activeChapterId.value = ch.id
    activeSectionId.value = sec.id
    expandedChapterId.value = ch.id
    await loadChapterDetail(ch.id)
    const detail = await sectionAPI.getSectionDetail(sec.id)
    currentSectionText.value = detail?.data?.text_content || ''
    await loadLinesBySection(sec.id)
    updateChapterRoleIds()
    saveLastChapter()
}

async function loadLinesBySection(sectionId) {
    const res = await lineAPI.getLinesBySection(sectionId)
    if (res?.code === 200) {
        lines.value = res.data || []
        stats.value.lineCount = lines.value.length
        lines.value.forEach(row => {
            if (row.audio_path) bumpVer(row.id)
        })
    }
}

function openNewSectionDialog(ch) {
    currentChapterForSection.value = ch
...
async function handleCreateSection() {
    ...
    if (!title || !currentChapterForSection.value) return
    if (sectionEditingId.value) {
        const res = await sectionAPI.renameSection(sectionEditingId.value, title)
        if (res?.code === 200) {
            ElMessage.success('已重命名')
        ...
        const res = await sectionAPI.createSection({ title, chapter_id: currentChapterForSection.value.id })
        if (res?.code === 200) {
            ElMessage.success('已创建分段')
            sectionDialogVisible.value = false
            await loadSections(currentChapterForSection.value.id)
        } else {
            ElMessage.error(res?.message || '创建失败')
...
function openRenameSection(sec) {
    currentChapterForSection.value = chapters.value.find(c => c.id === sec.chapter_id)
    sectionForm.value.title = sec.title
    sectionEditingId.value = sec.id
    sectionDialogVisible.value = true
}

async function handleDeleteSection(sec, ch) {
    const res = await sectionAPI.deleteSection(sec.id)
    if (res?.code === 200) {
        ElMessage.success('已删除分段')
        if (activeSectionId.value === sec.id) {
            activeSectionId.value = null
            currentSectionText.value = ''
        }
...
async function autoSplit() {
    ...
    try {
        const res = await chapterAPI.autoSplitChapter(activeChapterId.value)
        if (res?.code === 200) {
            ElMessage.success(`已创建 ${res.data?.length || 0} 个分段`)
            await loadSections(activeChapterId.value)
            expandedChapterId.value = activeChapterId.value
        } else {
            ElMessage.error(res?.message || '自动分段失败')
        }
    } catch (e) {
        ElMessage.error('自动分段失败')
    }
}
...
async function submitImport() {
    ...
    if (activeSectionId.value) {
        const res = await sectionAPI.updateSectionContent(activeSectionId.value, { text_content: text })
        if (res?.code === 200) {
            currentSectionText.value = text
            ElMessage.success('已导入分段正文')
...
async function splitByLLM() {
    ...
    try {
        await ElMessageBox.confirm(
            activeSectionId.value
                ? '确定要调用 LLM 对该分段进行台词拆分吗？此操作将覆盖该分段原有台词。'
    ...
    if (activeSectionId.value) {
        // 删除分段下的台词
        const existingLines = await lineAPI.getLinesBySection(activeSectionId.value)
        if (existingLines?.code === 200 && existingLines.data?.length) {
            for (const l of existingLines.data) {
                await lineAPI.deleteLine(l.id)
            }
        }
    } else {
    ...
    try {
        console.log('llm进行台词拆分请求开始', projectId, activeChapterId.value, activeSectionId.value)
        const res = await chapterAPI.splitChapterByLLM(projectId, activeChapterId.value, activeSectionId.value || null)
        if (res?.code === 200) {
...
async function loadLines() {
    ...
    let res
    if (activeSectionId.value) {
        res = await lineAPI.getLinesBySection(activeSectionId.value)
    } else {
        res = await lineAPI.getLinesByChapter(activeChapterId.value)
    }
...
async function updateChapterRoleIds() {
    if (!activeChapterId.value) return
    const res = await lineAPI.getLinesByChapter(activeChapterId.value)
    if (res?.code === 200) {
        const ids = new Set()
        res.data.forEach(l => { if (l.role_id) ids.add(l.role_id) })
        chapterRoleIds.value = ids
    }
}
...
const roleFilterMode = ref('all') // 'all' | 'chapter'
const chapterRoleIds = ref(new Set())
const chapterRoles = computed(() => roles.value.filter(r => chapterRoleIds.value.has(r.id)))

const displayedRoles = computed(() => {
    const kw = roleKeyword.value.trim().toLowerCase()
    let list = roles.value
    if (roleFilterMode.value === 'chapter') {
        list = list.filter(r => chapterRoleIds.value.has(r.id))
    }
    return list.filter(r => r.name.toLowerCase().includes(kw)).sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at))
})
...
onMounted(async () => {
    ...
    expandedChapterId.value = activeChapterId.value // 定位展开当前章节
    if (activeChapterId.value) {
        await loadSections(activeChapterId.value)
    }
    await loadLines()
    await updateChapterRoleIds()
    await loadChapterDetail(activeChapterId.value)
    // —— WebSocket：恢复历史队列并连接
    restoreQueue()
    connectWS()
...
// 批量替换角色
const dialogReplaceRole = ref(false)
const replaceRoleFormRef = ref(null)
const replaceRoleForm = ref({ from_role_id: null, to_role_id: null })

function openReplaceRole() {
    replaceRoleForm.value = { from_role_id: null, to_role_id: null }
    dialogReplaceRole.value = true
}

async function handleReplaceRole() {
    if (!activeChapterId.value) return
    const { from_role_id, to_role_id } = replaceRoleForm.value
    if (!from_role_id || !to_role_id) return ElMessage.warning('请选择两个角色')
    if (from_role_id === to_role_id) return ElMessage.warning('不能替换为同一个角色')

    try {
        await ElMessageBox.confirm(
            `确定将当前章节中「${roles.value.find(r => r.id === from_role_id)?.name}」的所有台词替换为「${roles.value.find(r => r.id === to_role_id)?.name}」吗？`,
            '确认替换',
            { confirmButtonText: '确定替换', cancelButtonText: '取消', type: 'warning' }
        )
    } catch { return }

    const res = await lineAPI.batchReplaceRole({
        chapter_id: activeChapterId.value,
        from_role_id,
        to_role_id,
        section_id: activeSectionId.value || null
    })
    if (res?.code === 200) {
        ElMessage.success(`已替换 ${res.data} 条台词`)
        dialogReplaceRole.value = false
        await loadLines()
        await loadRoles()
    } else {
...
async function markAllAsCompleted() {
    ...
    if (fail1 === 0 && fail2 === 0) {
        ...
        try {
            ...
            const exportDir = await native?.selectDir?.()
            if (!exportDir) {
                loading.close()
                ElMessage.info('已取消导出')
                return
            }
            const expRes = await lineAPI.exportLines(activeChapterId.value, isExportSingleSubtitle, exportDir)
...
.chapter-list-container :deep(.el-collapse) {
    border: none;
    --el-collapse-header-height: 40px;
}

.chapter-list-container :deep(.el-collapse-item__header) {
    padding: 0 8px;
    border: none;
    border-radius: 6px;
    margin-bottom: 2px;
...
.chapter-list-container :deep(.el-collapse-item__header:hover) {
    background: var(--el-fill-color-light);
}

.chapter-list-container :deep(.el-collapse-item__wrap) {
    border: none;
    padding: 0 8px;
}

.chapter-list-container :deep(.el-collapse-item__content) {
    padding: 8px 0;
}
...
.chapter-collapse-title.is-active {
    color: var(--el-color-primary);
    font-weight: 600;
}

.chapter-collapse-title .el-icon {
    flex-shrink: 0;
}
...
.section-item:hover .section-ops {
    opacity: 1;
}
