<template>
    ...
    >
      ...
      <el-table-column label="播放" width="160" align="center">
        <template #default="{ row }">
          <el-button
            size="small"
            :type="row.absolute_reference_path ? 'primary' : 'default'"
            :plain="!row.absolute_reference_path"
            :disabled="!row.absolute_reference_path"
            @click="togglePlay(row.absolute_reference_path)"
          >
            <el-icon style="margin-right:4px;">
              <Headset />
            </el-icon>
            {{ isPlaying && currentPath === row.absolute_reference_path ? '暂停' : '播放' }}
          </el-button>
        </template>
      </el-table-column>
  ...
  >
    ...
    />
        ...
        <el-form-item label="参考音频">
          ...
          <div class="preview" v-if="form.reference_path">
            ...
            <el-button type="primary" size="small" @click="togglePlay(form.absolute_reference_path || form.reference_path)">
              {{ isPlaying && currentPath === (form.absolute_reference_path || form.reference_path) ? '暂停' : '播放' }}
            </el-button>
          </div>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="音频编辑" v-model="audioEditorVisible" width="900px" :close-on-click-modal="false">
      ...
      <div class="wave-editor-wrap" v-if="editingVoice?.absolute_reference_path && waveEditorKey">
        <WaveCellPro
          :key="waveEditorKey"
          :src="editingVoice.absolute_reference_path"
          :speed="1.0"
          :volume2x="1.0"
...
function openDialog(row) {
  if (row) {
    form.value = {
      ...
      absolute_reference_path: row.absolute_reference_path || '',
...
async function pickLocalAudioForBase() {
  ...
  form.value.absolute_reference_path = p
}

function clearReferencePath() {
  form.value.reference_path = ''
  form.value.absolute_reference_path = ''
}
