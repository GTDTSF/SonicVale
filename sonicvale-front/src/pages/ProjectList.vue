                ...
                <el-card shadow="hover" class="project-card" :body-style="{ padding: '0px' }">
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-header-actions">
                            <el-button link size="small" @click="handleMoveUp(item)" title="上移">
                                <el-icon><ArrowUp /></el-icon>
                            </el-button>
                            <el-button link size="small" @click="handleMoveDown(item)" title="下移">
        ...
        <el-dialog title="创建新项目" v-model="dialogVisible" width="500px">
            <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
                ...
                <el-form-item label="文件夹名称" prop="folder_name">
                    <el-input v-model="form.folder_name" placeholder="请输入文件夹名称"></el-input>
                </el-form-item>
...
import { fetchProjects, createProject, deleteProject, reorderProject } from '../api/project'
import { fetchLLMProviders, fetchTTSProviders } from '../api/provider'
import { fetchPromptList } from '../api/prompt'
import { fetchSettings, updateSettings } from '../api/settings'
import { Plus, Delete, Cpu, Mic, Document, Clock, CircleCheck, CircleClose, ArrowUp, ArrowDown } from "@element-plus/icons-vue"
const prompts = ref([])
...
const handleMoveUp = async (item) => {
    try {
        const res = await reorderProject(item.id, 'up')
        if (res?.code === 200) {
            projects.value = await fetchProjects()
        } else {
...
const handleMoveDown = async (item) => {
    try {
        const res = await reorderProject(item.id, 'down')
        if (res?.code === 200) {
            projects.value = await fetchProjects()
        } else {
