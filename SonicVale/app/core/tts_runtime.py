...
from app.core.config import resolve_voice_path, resolve_path
...
async def tts_worker(app: FastAPI):
    ...
    while True:
        ...
        try:
            ...
            if not role:
                raise Exception(f"未找到角色 (ID: {dto.role_id})，可能已被删除")
            if not role.default_voice_id:
                raise Exception(f"角色「{role.name}」未绑定音色，请先绑定音色后再生成")
            voice = voice_service.get_voice(role.default_voice_id)
            if not voice:
                raise Exception(f"音色不存在 (ID: {role.default_voice_id})，可能已被删除")
            reference_path = resolve_voice_path(voice.reference_path)
            ...
            audio_path_abs = resolve_path(dto.audio_path)
            ...
            logging.exception("⚠️ TTS 生成失败: line_id=%s, error=%s", dto.id, e)
            try:
                line_service.update_line(dto.id, {"status": "failed"})
            except Exception as ex:
                logging.error("更新失败状态出错: %s", ex)
            try:
                await manager.broadcast({
                    "event": "line_update",
                    "line_id": dto.id,
