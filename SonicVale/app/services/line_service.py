...
from app.core.config import getFfmpegPath, resolve_path, make_path_relative
...
class LineService:
    ...
    def get_line(self, line_id: int) -> LineEntity | None:
        ...
        res = LineEntity(**data)
        if res.audio_path:
            res.audio_path = resolve_path(res.audio_path)
        return res
    ...
    def get_all_lines(self,chapter_id: int) -> Sequence[LineEntity]:
        ...
        for e in entities:
            if e.audio_path:
                e.audio_path = resolve_path(e.audio_path)
        return entities

    def get_lines_by_section(self, section_id: int) -> Sequence[LineEntity]:
        """获取分段下的台词列表"""
        pos = self.repository.get_by_section(section_id)
        entities = [
            LineEntity(**{k: v for k, v in po.__dict__.items() if not k.startswith("_")})
            for po in pos
        ]
    ...
    def batch_replace_role(self, chapter_id: int, from_role_id: int, to_role_id: int, section_id: int = None) -> int:
        return self.repository.batch_replace_role(chapter_id, from_role_id, to_role_id, section_id)
    ...
    def delete_line(self, line_id: int) -> bool:
        ...
        if po and po.audio_path:
            with contextlib.suppress(FileNotFoundError):
                os.remove(resolve_path(po.audio_path))
    ...
    def delete_all_lines(self, chapter_id: int) -> bool:
        ...
        for line in self.get_all_lines(chapter_id):
            if line and line.audio_path:
                with contextlib.suppress(FileNotFoundError):
                    os.remove(resolve_path(line.audio_path))
        return self.repository.delete_all_by_chapter_id(chapter_id)

    # 单个台词新增
    def add_new_line(self, line: LineCreateDTO,project_id,chapter_id,index,emotions_dict, strengths_dict,audio_path,section_id=None):
    #     先判断角色是否存在
        ...
        self.repository.update(res.id, {"audio_path": make_path_relative(res_path)})


    def update_init_lines(self, lines: list, project_id: object, chapter_id: object,emotions_dict, strengths_dict,audio_path, section_id=None) -> None:
        for index, line in enumerate(lines):
            self.add_new_line(line,project_id,chapter_id,index,emotions_dict, strengths_dict,audio_path, section_id=section_id)
    ...
    def update_line(self, line_id: int, data: dict) -> bool:
        ...
        if "audio_path" in data and data["audio_path"]:
            data["audio_path"] = make_path_relative(data["audio_path"])
    ...
    def update_audio_path(self, id, dto) -> bool:
        try:
            po = self.get_line(id)
            old_path = resolve_path(po.audio_path)
            new_path = resolve_path(dto.audio_path)
            ...
            # 更新数据库
            self.update_line(id, {"audio_path": make_path_relative(new_path)})
            return True
    ...
    def process_audio(self, line_id, dto:LineAudioProcessDTO):
        ...
        #     audio_file = self.process_audio_ffmpeg_cut(line.audio_path, dto.speed, dto.volume, dto.start_ms, dto.end_ms, dto.tail_silence_sec,dto.current_ms)
            processor = AudioProcessor(resolve_path(line.audio_path))
    ...
    def export_audio(self, chapter_id, single=False, export_path=None):
        """导出音频与字幕
        
        Args:
            chapter_id: 章节ID
            single: 是否同时导出单条字幕
            export_path: 导出目标文件夹路径（如果为None，使用旧的自动路径）
        ...
        try:
            ...
            for line in lines:
                ...
                elif not os.path.exists(resolve_path(line.audio_path)):
            ...
            paths = [resolve_path(line.audio_path) for line in valid_lines]
            
            if export_path:
                output_dir_path = export_path
            else:
            ...
            if single:
                if export_path:
                    subtitle_dir_path = os.path.join(export_path, "subtitles")
                else:
                ...
                for line in valid_lines:
                    try:
                        path = resolve_path(line.audio_path)
    ...
    def generate_subtitle(self, line_id, dto):
        ...
        if line:
            ...
            subtitle_engine.generate_subtitle(resolve_path(line.audio_path),dto.subtitle_path)
