...
from app.dto.section_dto import SectionResponseDTO
from app.dto.project_dto import ProjectReorderDTO
...
# 修改，传入的参数是id
@router.put("/reorder", response_model=Res,
            summary="调整章节顺序",
            description="将章节向上或向下移动")
def reorder_chapter(dto: ProjectReorderDTO, chapter_service: ChapterService = Depends(get_chapter_service)):
    success = chapter_service.repository.swap_order_index(dto.project_id, dto.direction)
    if success:
        return Res(data=None, code=200, message="排序成功")
    else:
        return Res(data=None, code=400, message="排序失败，可能已到边界")
...
@router.post("/{chapter_id}/auto-split", response_model=Res,
             summary="自动分段",
             description="将章节内容按约5000字自动拆分为多个子分段")
def auto_split_chapter(chapter_id: int,
                        chapter_service: ChapterService = Depends(get_chapter_service),
    ...
    if not content:
        return Res(data=None, code=400, message="章节内容为空")

    # 先删除已有的分段
    existing = section_service.get_all_sections(chapter_id)
    for s in existing:
        section_service.delete_section(s.id)

    paragraphs = [p.strip() for p in content.split('\n') if p.strip()]
    chunks = []
    buf = ""
    for p in paragraphs:
        if len(buf) + len(p) + 1 <= 5000:
            buf = (buf + '\n' + p) if buf else p
        else:
            if buf:
                chunks.append(buf)
    ...
    for idx, chunk in enumerate(chunks, start=1):
        entity = SectionEntity(chapter_id=chapter_id, title=f"第{idx}段", text_content=chunk)
        res = section_service.create_section(entity)
        if res:
            created.append(res)
            logging.info("自动创建分段: 章节=%s, 分段=%s, 字数=%s", chapter.title, res.title, len(chunk))

    # 将已有的 chapter 级别台词 (section_id = NULL) 按文本匹配分配到新分段
    lines = line_service.get_all_lines(chapter_id)
    reassigned = 0
    for line in lines:
        if line.section_id is not None:
            continue
        line_text = (line.text_content or "").strip()
        if not line_text:
            continue
        for section in created:
            if line_text in (section.text_content or ""):
                line_service.update_line(line.id, {"section_id": section.id})
                reassigned += 1
                break
    if reassigned > 0:
        logging.info("自动分段: 已将 %s 条台词分配到新分段", reassigned)

    return Res(data=[SectionResponseDTO(**s.__dict__) for s in created],
               code=200, message=f"已创建 {len(created)} 个分段")
...
async def get_lines(
    project_id: int,
    chapter_id: int,
    section_id: int = Query(None, description="分段ID，传入则使用分段内容"),
    chapter_service: ChapterService = Depends(get_chapter_service),
    line_service: LineService = Depends(get_line_service),
    ...
    if section_id:
        ...
        if not content:
            return Res(data=None, code=400, message="分段内容为空")
    else:
        chapter = chapter_service.get_chapter(chapter_id)
    ...
    try:
        if section_id:
            # 按段落拆分分段内容
            parags = [p.strip() for p in content.split('\n') if p.strip()]
            contents = []
            buf = ""
            for p in parags:
                if len(buf) + len(p) + 1 <= 1500:
                    buf = (buf + '\n' + p) if buf else p
                else:
                    if buf:
                        contents.append(buf)
    ...
    try:
        audio_path = os.path.join(get_project_root(project.folder_name),str(project_id),str(chapter.order_index),"audio")
        os.makedirs(audio_path, exist_ok=True)
        line_service.update_init_lines(
            all_line_data, project_id, chapter_id, emotions_dict, strengths_dict,audio_path, section_id=section_id
        )
    except Exception as e:
...
async def import_lines(project_id: int,chapter_id: int,data:str=Form( ...),line_service: LineService = Depends(get_line_service),
    ...
    audio_path = os.path.join(get_project_root(project.folder_name),str(project_id),str(chapter.order_index),"audio")
    os.makedirs(audio_path, exist_ok=True)
    line_service.update_init_lines(lines_data, project_id, chapter_id, emotions_dict, strengths_dict,audio_path, section_id=section_id)
