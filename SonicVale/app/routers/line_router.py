...
from app.core.config import resolve_path, make_path_relative, get_project_root
...
def create_line(project_id:int,dto: LineCreateDTO, line_service: LineService = Depends(get_line_service),
    ...
    try:
        ...
        line_service.update_line(entityRes.id, {"audio_path": make_path_relative(res_path)})
...
@router.get("/section/{section_id}", response_model=Res[List[LineResponseDTO]],
            summary="查询分段下的所有台词",
            description="根据分段id查询分段下的所有台词信息")
def get_lines_by_section(section_id: int, line_service: LineService = Depends(get_line_service)):
    entities = line_service.get_lines_by_section(section_id)
    res = [LineResponseDTO(**e.__dict__) for e in entities]
    return Res(data=res, code=200, message="查询成功")
...
class BatchReplaceRoleDTO(BaseModel):
    chapter_id: int
    from_role_id: int
    to_role_id: int
...
@router.put("/batch/replace-role", response_model=Res[int],
            summary="批量替换角色",
            description="将章节中某个角色的所有台词替换为另一个角色")
def batch_replace_role(dto: BatchReplaceRoleDTO, line_service: LineService = Depends(get_line_service)):
    count = line_service.batch_replace_role(dto.chapter_id, dto.from_role_id, dto.to_role_id, dto.section_id)
    return Res(data=count, code=200, message=f"已替换 {count} 条台词")
...
# 导出音频与字幕
class ExportAudioDTO(BaseModel):
    export_dir: str
    single: bool = False

@router.post("/export-audio/{chapter_id}")
async def export_audio(chapter_id: int,
    ...
    order_str = str(chapter.order_index).zfill(2) if chapter.order_index else "00"
    safe_title = re.sub(r'[<>:"/\\|?*]', '_', chapter.title or "untitled")
    folder_name = f"{order_str}_{safe_title}"
    export_path = os.path.join(dto.export_dir, folder_name)
    os.makedirs(export_path, exist_ok=True)
    res = line_service.export_audio(chapter_id, dto.single, export_path)
...
async def correct_subtitle_pinyin(
    ...
    paths = [resolve_path(line.audio_path) for line in lines]
