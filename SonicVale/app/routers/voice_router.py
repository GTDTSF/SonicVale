...
from app.core.config import resolve_voice_path
...
def to_voice_response(entity):
    d = entity.__dict__.copy()
    d['absolute_reference_path'] = resolve_voice_path(entity.reference_path)
    return VoiceResponseDTO(**d)
...
def copy_voice(dto: VoiceCopyDTO, voice_service: VoiceService = Depends(get_voice_service)):
    ...
    try:
        ...
        res = to_voice_response(new_voice)
...
def get_all_voices(tts_provider_id: int, voice_service: VoiceService = Depends(get_voice_service)):
    ...
    if entities:
        res = [to_voice_response(e) for e in entities]
...
def create_voice(dto: VoiceCreateDTO, voice_service: VoiceService = Depends(get_voice_service),
    ...
    try:
        ...
        if entityRes is not None:
            ...
            res = to_voice_response(entityRes)
