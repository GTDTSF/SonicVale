...
from app.dto.project_dto import ProjectCreateDTO, ProjectResponseDTO, ProjectReorderDTO, ProjectImportDTO
...
@router.put("/reorder", response_model=Res,
            summary="调整项目顺序",
            description="将项目向上或向下移动")
def reorder_project(dto: ProjectReorderDTO, service: ProjectService = Depends(get_service)):
    success = service.reorder_project(dto.project_id, dto.direction)
    if success:
...
def delete_project(project_id: int, service: ProjectService = Depends(get_service), chapter_service: ChapterService = Depends(get_chapter_service),role_service: RoleService = Depends(get_role_service)):
    ...
    project_path = os.path.join(get_project_root(project.folder_name), str(project_id))
