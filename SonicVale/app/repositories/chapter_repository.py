...
class ChapterRepository:
    ...
    def get_all(self, project_id: int) -> Sequence[ChapterPO]:
        """获取指定项目下的所有章节"""
        stmt = (select(ChapterPO)
                .where(ChapterPO.project_id == project_id)
                .order_by(ChapterPO.order_index.asc()))
    ...
    def get_max_order_index(self, project_id: int) -> int:
        stmt = (select(ChapterPO)
                .where(ChapterPO.project_id == project_id)
                .order_by(ChapterPO.order_index.desc())
                .limit(1))
        po = self.db.execute(stmt).scalar_one_or_none()
        return po.order_index if po else 0

    def swap_order_index(self, chapter_id: int, direction: str) -> bool:
        current = self.get_by_id(chapter_id)
        if not current:
            return False
        current_order = current.order_index or 0
        if direction == "up":
            stmt = (select(ChapterPO)
                    .where(ChapterPO.project_id == current.project_id)
                    .where(ChapterPO.order_index < current_order)
                    .order_by(ChapterPO.order_index.desc())
                    .limit(1))
        elif direction == "down":
            stmt = (select(ChapterPO)
        ...
        current.order_index = neighbor_order
